...
> 2026-08-09 Go service CLI compatibility：Rust binary 现在接受 Go service command 使用的 `-host/-path/-u/-p/-eweb/-nfs-mode` 参数，也支持 `run`、`version`、`help` 和 `update-helper` 入口。`-path DIR` 按 Go `paths.PathGenerator.State` 使用 `DIR/state.db`，默认 HTTP 地址改为 `0.0.0.0:50051`；`YUHAIIN_DB`/`YUHAIIN_HTTP` 仍可用于测试和显式覆盖。这样 systemd/launchd 直接替换 executable 时不会因 flag、端口或 state 文件名不一致而启动到另一套运行环境；服务安装、backup/rollback 和旧 Go 并行切换仍需现场演练。
