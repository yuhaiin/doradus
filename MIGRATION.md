...
> 2026-08-09 cross-target boundary：`yuhaiin-core` 的 `async-proxy,tun` 已通过 `aarch64-linux-android` 和 `aarch64-apple-darwin` 的 `cargo check`，说明共享 TUN/Flow/Proxy 源码没有目标平台条件编译错误。完整 runtime target check 被 bundled `libsqlite3-sys` 的宿主工具链阻断：Android 缺少 `aarch64-linux-android-clang`，macOS 使用 Linux `cc` 无法处理 `-arch/-mmacosx-version-min`；这需要对应 NDK/Apple SDK，不能冒充源代码失败或平台实机验收。
