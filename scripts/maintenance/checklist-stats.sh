...
declared_total="$(awk -F'|' '$2 ~ /tracked module acceptance items/ {gsub(/[[:space:]]/, "", $3); print $3; exit}' "${checklist}")"
declared_completed="$(awk -F'|' '$2 ~ /completed/ {gsub(/[[:space:]]/, "", $3); print $3; exit}' "${checklist}")"
declared_partial="$(awk -F'|' '$2 ~ /main path works but still has field issues/ {gsub(/[[:space:]]/, "", $3); print $3; exit}' "${checklist}")"
declared_coverage="$(sed -n 's/^| Weighted coverage | \*\*\([0-9.]*\)%\*\*.*/\1/p' "${checklist}" | head -n 1)"
