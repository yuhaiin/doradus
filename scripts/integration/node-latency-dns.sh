...
scenario_dir="${YUHAIIN_INTEGRATION_DIR:-${cache_root}/integration/node-latency-dns}"
image="${YUHAIIN_TEST_IMAGE:-docker.io/library/debian:testing}"

mkdir -p "${scenario_dir}"

echo "[node-latency-dns] building runtime test binary in Podman"
...
echo "[node-latency-dns] running API DNS latency chain in Podman"
podman run --rm \
  --network=none \
  ...
  -ec '
    ...
    /usr/local/bin/yuhaiin-runtime-test \
      api::tests::direct_node_latency_dns_uses_the_selected_proxy_datagram \
      --exact --nocapture
  ' \
  | tee "${scenario_dir}/podman.log"

grep -q 'test result: ok' "${scenario_dir}/podman.log"
echo "[node-latency-dns] passed; logs=${scenario_dir}"
