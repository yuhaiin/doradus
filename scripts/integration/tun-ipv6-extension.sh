...
scenario_dir="${DORADUS_INTEGRATION_DIR:-${cache_root}/integration/tun-ipv6-extension}"
...
podman run --rm \
  ...
  -ec '
    ...
    for test_name in \
      ...
      /usr/local/bin/doradus-core-test "${test_name}" --exact --nocapture
    done
  ' \
...
DORADUS_INTEGRATION_DIR="${scenario_dir}/kernel" \
DORADUS_TUN_IPV6_EXTENSION=1 \
DORADUS_TUN_PORTAL_V6="fd00:253::1/64" \
DORADUS_TUN_ROUTE="fd00:253::2/128" \
DORADUS_TUN_IPV6_SOURCE="fd00:253::1" \
DORADUS_TUN_IPV6_TARGET="[fd00:253::2]:18080" \
DORADUS_TUN_UDP_TRAFFIC=1 \
DORADUS_TUN_UDP_TRAFFIC_BYTES=32 \
