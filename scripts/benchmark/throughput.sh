...
scenario_dir="${DORADUS_BENCH_DIR:-${cache_root}/benchmarks/http-throughput}"
image="${DORADUS_TEST_IMAGE:-docker.io/library/debian:testing}"
bytes="${DORADUS_BENCH_BYTES:-67108864}"
...
test -n "${test_binary}"
runtime_binary="${target_dir}/release/doradus"
test -x "${runtime_binary}"

echo "[throughput] running one-stream loopback benchmark in Podman"
podman run --rm \
  --network=host \
  -v "${test_binary}:/usr/local/bin/doradus-throughput:ro" \
  ...
  -e DORADUS_BENCH_BYTES="${bytes}" \
  -e DORADUS_BENCH_PROFILE=release \
  --entrypoint /bin/sh \
  "${image}" \
  -ec '
    set -eu
    for test_name in \
      ...
      /usr/local/bin/doradus-throughput "${test_name}" --exact --ignored --nocapture
    done
  ' \
