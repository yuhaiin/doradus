...
help:
	@printf '%s\n' \
		...
		'make release-linux-cross-smoke check Linux musl target with the pinned cross toolchain' \
		'make release-windows-cross-smoke check Windows cfg/dependencies with MinGW in Podman' \
		...
		'make release-contract-smoke validate the six-platform GitHub release contract' \
...
release-linux-cross-smoke:
	./scripts/integration/release-linux-cross.sh

release-windows-cross-smoke:
	./scripts/integration/release-windows-cross.sh
...
wireguard-chain-smoke:
	./scripts/integration/wireguard-chain.sh
