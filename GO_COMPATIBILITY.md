...
状态：P1 fixture/import 基线，2026-08-08；已补 production-shaped snapshot、同名 legacy table 处理、Go v1 legacy 显式升级、Rust schema v3 Geo route 字段，以及 Go v6 typed compatibility views 的事务性读写回和全 compatibility JSON 边界 fail-closed 校验。

兼容版本边界：Rust 只接受已审计的 Go schema version `0..=6`；更高版本会 fail-closed，待对应 plain-contract 迁移和字段差异审计完成后再放行，避免把未来 Go 数据按旧 schema 静默降级读取。
