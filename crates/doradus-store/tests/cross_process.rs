...
use doradus_store::fakeip::{FakeIpConfig, FakeIpPool, FakeIpPoolOptions};
...
fn database_path() -> PathBuf {
    ...
    let directory = cache.join("doradus-check");
...
fn force_stopped_fakeip_transaction_keeps_committed_mapping_and_discards_row() {
    ...
    let path = database_path();
    let worker = worker_path();
    let committed_domain = doradus_core::DomainName::new("committed.example").unwrap();
    let config =
    ...
    assert_eq!(
        block_on(pool.lookup_ip(&doradus_core::DomainName::new("uncommitted.example").unwrap())),
        None
    );
