...
fn imports_go_v1_nodes_inbounds_lists_and_tags_into_v2_contracts() {
    ...
    let reopened = block_on(ConfigStore::open_legacy(&path)).unwrap();
    assert_eq!(
...
#[test]
#[ignore = "requires DORADUS_GO_LEGACY_PRODUCTION_DB pointing to a copied Go v1 state.db"]
fn imports_real_go_v1_snapshot_without_touching_source() {
    let source = std::env::var_os("DORADUS_GO_LEGACY_PRODUCTION_DB")
        .map(PathBuf::from)
        .expect("DORADUS_GO_LEGACY_PRODUCTION_DB must point to a Go v1 snapshot");
    ...
    let store = block_on(ConfigStore::open_legacy(&path)).expect("open Go v1 snapshot with Rust");
    let repository = store.repository();
    assert_eq!(
...
fn each_go_v6_import_table_failure_rolls_back_all_typed_rows() {
    ...
    for (table, corrupt, repair) in cases {
        ...
        assert!(
            block_on(ConfigStore::open_legacy(&path)).is_err(),
        ...
        );
        {
            ...
            assert!(
                connection
                    .query("SELECT key FROM doradus_config WHERE key LIKE 'go.%'")
                    .unwrap()
                    .is_empty(),
    ...
    assert!(block_on(ConfigStore::open_legacy(&path)).is_err());
    {
        let connection = Connection::open(path.to_str().unwrap()).unwrap();
        assert!(!meta_flag(&connection, "go_schema_imported"));
        assert_eq!(table_row_count(&connection, "proxy_nodes").unwrap(), 0);
...
#[test]
#[ignore = "requires DORADUS_GO_PRODUCTION_DB pointing to a consistent FTS-free Go SQLite export"]
fn imports_real_go_production_snapshot_without_touching_source() {
    let source = std::env::var_os("DORADUS_GO_PRODUCTION_DB")
        .map(PathBuf::from)
        .expect("DORADUS_GO_PRODUCTION_DB must point to a copied/consistent Go snapshot");
    assert!(
        source.is_file(),
    ...
    let store = block_on(ConfigStore::open_legacy(&path)).expect("open real Go snapshot with Rust");
    let repository = store.repository();
...
fn opens_native_go_v5_database_directly_and_keeps_source_unchanged() {
    let source = std::env::var_os("DORADUS_GO_NATIVE_DB")
        .map(PathBuf::from)
        .expect("DORADUS_GO_NATIVE_DB must point to a native Go database");
    ...
    let store =
        block_on(ConfigStore::open_legacy(&path)).expect("open native Go database directly");
    let repository = store.repository();
...
fn opens_native_go_v6_database_directly_and_keeps_source_unchanged() {
    let source = std::env::var_os("DORADUS_GO_NATIVE_V6_DB")
        .map(PathBuf::from)
        .expect("DORADUS_GO_NATIVE_V6_DB must point to a native Go v6 database");
    ...
    let store =
        block_on(ConfigStore::open_legacy(&path)).expect("open native Go v6 database directly");
    let repository = store.repository();
...
fn imports_production_shaped_go_snapshot_without_losing_legacy_tables() {
...

    {
        ...
        assert_eq!(
            hosts
                .resolve(&doradus_core::DomainName::new("legacy.example").unwrap())
                .unwrap()
                .unwrap()
                .v4,
