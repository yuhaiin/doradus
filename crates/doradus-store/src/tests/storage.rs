...
fn sqlite_backup_failure_removes_sanitization_staging_files() {
    ...
    let staging_prefix = format!(".{backup_name}.doradus-backup-");
...
fn schema_is_idempotent() {
    let store = block_on(ConfigStore::open_memory()).unwrap();
    let second = store.migrate(false);
    assert!(second.is_ok());
}
...
fn storage_status_is_reusable_by_reload_and_management_callers() {
    ...
    assert_eq!(status.go_schema_version, None);
...
fn wal_mode_and_uncommitted_force_stop_recover_on_reopen() {
    ...
    }
    {
        ...
        connection
            .execute_with_params(
                "INSERT OR REPLACE INTO doradus_config (key, value) VALUES (?1, ?2)",
                &[
