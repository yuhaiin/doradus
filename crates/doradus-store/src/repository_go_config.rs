...
impl ConfigRepository {
    ...
    /// native Doradus configuration uses JSON objects in `doradus_config`.
    ...
    pub async fn get_go_selected_node_id(&self, key: &str) -> Result<Option<String>> {
        ...
        if table_exists(&connection, "doradus_config") {
            let rows = connection
                .query_with_params(
                ...
                .map_err(storage_error)?;
            if let Some(row) = rows.first() {
                let value = row_blob(row, 0, "doradus_config.value")?;
                let selected = serde_json::from_slice::<serde_json::Value>(&value)
                    .ok()
                    .and_then(|value| {
                        value
                            .get("id")
