...
use doradus_core::dns_http::{DnsOverHttp, DnsOverHttpConnector, HttpConnection, HttpVersion};
...
use doradus_tun::{TunConfig, TunDispatcher, TunProxyRuntime, TunRuntime};
...
fn database_path() -> io::Result<PathBuf> {
    if let Some(path) = env::var_os("DORADUS_TUN_FAKEIP_DB") {
        return Ok(PathBuf::from(path));
    }
    Ok(PathBuf::from(".cache/doradus-check")
...
async fn async_main() -> io::Result<()> {
    ...
    let mut tun = TunRuntime::open(TunConfig {
        name: env::var("DORADUS_TUN_NAME").ok(),
        ipv4: Some((
            "10.0.0.1"
                .parse()
