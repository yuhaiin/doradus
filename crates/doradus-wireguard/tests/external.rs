...
use doradus_core::{DomainName, Endpoint, Error, ErrorKind, FlowContext, Network, Result};
use doradus_wireguard::{WireGuardConfig, build_proxy};
...
async fn external_proxy() -> Result<doradus_wireguard::WireGuardProxy> {
    let config_path = required_env("DORADUS_WIREGUARD_EXTERNAL_CONFIG");
...
async fn external_peer_tcp_connects() {
    let target_raw = match std::env::var("DORADUS_WIREGUARD_EXTERNAL_TCP_TARGET") {
        Ok(target) => target,
        Err(_) => {
    ...
    let mut stream = tokio::time::timeout(Duration::from_secs(20), proxy.connect(&context))
        ...
        .unwrap()
        .unwrap();
    if let Ok(request) = std::env::var("DORADUS_WIREGUARD_EXTERNAL_TCP_REQUEST") {
        stream.write_all(request.as_bytes()).await.unwrap();
    }
    if let Ok(expected) = std::env::var("DORADUS_WIREGUARD_EXTERNAL_TCP_EXPECT") {
...
async fn external_peer_udp_round_trips() {
    let target_raw = match std::env::var("DORADUS_WIREGUARD_EXTERNAL_UDP_TARGET") {
        Ok(target) => target,
        Err(_) => {
    ...
    let datagram = tokio::time::timeout(Duration::from_secs(20), proxy.open_datagram(&context))
        ...
        .unwrap()
        .unwrap();
    let payload = std::env::var("DORADUS_WIREGUARD_EXTERNAL_UDP_PAYLOAD_HEX")
        .map(|value| {
    ...
    let expect_reply = std::env::var("DORADUS_WIREGUARD_EXTERNAL_UDP_EXPECT_REPLY")
        .map(|value| value != "0")
        .unwrap_or(true);
    if expect_reply {
