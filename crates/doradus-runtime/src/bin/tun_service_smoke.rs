...
async fn main() {
    if std::env::args().nth(1).as_deref() == Some("--traffic-client") {
        let total_bytes = match std::env::var("DORADUS_TUN_TRAFFIC_BYTES")
            .ok()
        ...
        let connection_hold_ms = std::env::var("DORADUS_TUN_CONNECTION_HOLD_MS")
            .ok()
    ...
    if std::env::args().nth(1).as_deref() == Some("--udp-traffic-client") {
        let total_bytes = std::env::var("DORADUS_TUN_UDP_TRAFFIC_BYTES")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(32);
...
async fn run() -> Result<()> {
    let database = std::env::var_os("DORADUS_DB")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(".cache/doradus/integration/tun-service/state.sqlite"));
    let name = std::env::var("DORADUS_TUN_NAME").unwrap_or_else(|_| "doradus-smoke0".to_owned());
    let hold_ms = std::env::var("DORADUS_TUN_HOLD_MS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or(750);
    let mtu = std::env::var("DORADUS_TUN_MTU")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(1500);
    if !(576..=9216).contains(&mtu) {
        return Err(Error::invalid(format!(
            "DORADUS_TUN_MTU must be between 576 and 9216, got {mtu}"
        )));
    }
    let traffic = std::env::var_os("DORADUS_TUN_TRAFFIC").is_some();
    let dns_test = std::env::var_os("DORADUS_TUN_DNS_TEST").is_some();
    let udp_traffic = std::env::var_os("DORADUS_TUN_UDP_TRAFFIC").is_some();
    let ipv6_extension = std::env::var_os("DORADUS_TUN_IPV6_EXTENSION").is_some();
    let udp_first = udp_traffic && std::env::var_os("DORADUS_TUN_UDP_FIRST").is_some();
    if udp_traffic && !traffic {
        return Err(Error::invalid(
            "DORADUS_TUN_UDP_TRAFFIC requires DORADUS_TUN_TRAFFIC",
        ));
    }
    if udp_traffic && chain_mode_is_set() {
        return Err(Error::invalid(
            "DORADUS_TUN_UDP_TRAFFIC is currently supported by the direct TUN fixture only",
        ));
    }
    if ipv6_extension && (!traffic || !udp_traffic) {
        return Err(Error::invalid(
            "DORADUS_TUN_IPV6_EXTENSION requires both DORADUS_TUN_TRAFFIC and DORADUS_TUN_UDP_TRAFFIC",
        ));
    }
    if ipv6_extension && udp_first {
        return Err(Error::invalid(
            "DORADUS_TUN_IPV6_EXTENSION cannot be combined with DORADUS_TUN_UDP_FIRST",
        ));
    }
    let traffic_bytes = std::env::var("DORADUS_TUN_TRAFFIC_BYTES")
        .ok()
        .map(|value| {
            value.parse::<usize>().map_err(|_| {
                Error::invalid(format!(
                    "DORADUS_TUN_TRAFFIC_BYTES must be a positive integer, got {value:?}"
                ))
            })
        })
        .transpose()?
    ...
    if traffic_bytes == 0 || traffic_bytes > 512 * 1024 * 1024 {
        return Err(Error::invalid(
            "DORADUS_TUN_TRAFFIC_BYTES must be between 1 and 536870912",
        ));
    }
    let udp_traffic_bytes = std::env::var("DORADUS_TUN_UDP_TRAFFIC_BYTES")
        .ok()
        .map(|value| {
            value.parse::<usize>().map_err(|_| {
                Error::invalid(format!(
                    "DORADUS_TUN_UDP_TRAFFIC_BYTES must be a positive integer, got {value:?}"
                ))
            })
        })
        .transpose()?
    ...
    if udp_traffic_bytes == 0 || udp_traffic_bytes > 65_507 {
        return Err(Error::invalid(
            "DORADUS_TUN_UDP_TRAFFIC_BYTES must be between 1 and 65507",
        ));
    }
    let connection_hold_ms = std::env::var("DORADUS_TUN_CONNECTION_HOLD_MS")
        .ok()
        .and_then(|value| value.parse::<u64>().ok())
        .unwrap_or_default();
    let assert_connections = std::env::var_os("DORADUS_TUN_ASSERT_CONNECTIONS").is_some();
    let assert_process = std::env::var_os("DORADUS_TUN_ASSERT_PROCESS").is_some();
    let reload_inbound = std::env::var_os("DORADUS_TUN_RELOAD").is_some();
    let reload_cycles = std::env::var("DORADUS_TUN_RELOAD_CYCLES")
        .ok()
        .map(|value| {
            value.parse::<usize>().map_err(|_| {
                Error::invalid(format!(
                    "DORADUS_TUN_RELOAD_CYCLES must be a positive integer, got {value:?}"
                ))
            })
        })
        .transpose()?
    ...
    if reload_cycles == 0 || reload_cycles > 32 {
        return Err(Error::invalid(
            "DORADUS_TUN_RELOAD_CYCLES must be between 1 and 32",
        ));
    }
    let chain_mode = std::env::var("DORADUS_TUN_CHAIN")
        .ok()
        .filter(|value| !value.trim().is_empty());
    let reset_reconnect = std::env::var_os("DORADUS_TUN_RESET_RECONNECT").is_some();
    if reset_reconnect && chain_mode.is_some() {
        return Err(Error::invalid(
            "DORADUS_TUN_RESET_RECONNECT is only supported by the direct TUN fixture",
        ));
    }

    if dns_test && chain_mode.is_some() {
        return Err(Error::invalid(
            "DORADUS_TUN_DNS_TEST is only supported by the direct TUN fixture",
        ));
    }
    ...
    if traffic || dns_test {
        doradus_tun::enable_loopback().map_err(io_error)?;
    }

    let (target_address, target_task, udp_target_task, chain_fixture) = if chain_mode.is_some() {
        if !traffic {
            return Err(Error::invalid(
                "DORADUS_TUN_CHAIN requires DORADUS_TUN_TRAFFIC",
            ));
        }
    ...
    if traffic {
        ...
        let mut traffic_client = std::process::Command::new(executable)
            .arg("--traffic-client")
            .env("DORADUS_TUN_TRAFFIC_BYTES", traffic_bytes.to_string())
            .env(
                "DORADUS_TUN_CONNECTION_HOLD_MS",
    ...
    if let Some(target_address) = target_address {
        let (node_id, chain_types_json, chain) = if let Some(outbound) = chain_outbound {
            if !chain_mode.is_some_and(|mode| mode.eq_ignore_ascii_case("tls-h2-yuubinsya")) {
                return Err(Error::invalid(
                    "unsupported DORADUS_TUN_CHAIN; expected tls-h2-yuubinsya",
                ));
            }
            (
