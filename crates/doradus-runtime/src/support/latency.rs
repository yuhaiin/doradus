...
use doradus_dns::{AsyncDnsDatagram, DnsDatagramConnector, probe_dns_udp};
#[cfg(feature = "doh-tls")]
use doradus_dns::{DoqResolverConfig, DoqResolverFactory, probe_doq};
...
impl DnsDatagramConnector for LatencyDatagramConnector {
    ...
    ) -> doradus_core::BoxFuture<'a, Result<Option<Box<dyn AsyncDnsDatagram>>>> {
        Box::pin(async move {
...
impl AsyncDnsDatagram for LatencyDatagram {
    fn send_to<'a>(
        ...
        payload: &'a [u8],
        _target: SocketAddr,
    ) -> doradus_core::BoxFuture<'a, Result<usize>> {
    ...
    fn recv_from<'a>(
        &'a self,
        buffer: &'a mut [u8],
    ) -> doradus_core::BoxFuture<'a, Result<(usize, SocketAddr)>> {
        Box::pin(async move {
