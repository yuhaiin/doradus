...
use doradus_store::GoRouteListRecord;
use std::sync::Arc;
...
fn all_matcher_requires_every_positive_host_constraint() {
    ...
    let matching = Endpoint::domain(
        Network::Tcp,
        doradus_core::DomainName::new("blocked.example.com").unwrap(),
        443,
    );
    let parent_only = Endpoint::domain(
        Network::Tcp,
        doradus_core::DomainName::new("other.example.com").unwrap(),
        443,
    );
    assert_eq!(
...
fn go_single_port_string_is_a_single_port_range() {
    ...
    assert_eq!(router.decide(&other).mode, doradus_core::RouteMode::Direct);
}
...
fn local_host_list_keeps_one_router_rule_and_shares_the_index() {
    ...
    let endpoint = Endpoint::domain(
        Network::Tcp,
        doradus_core::DomainName::new("www.example.com").unwrap(),
        443,
    );
    assert_eq!(
        router.decide(&endpoint).mode,
    ...
    let wildcard = Endpoint::domain(
        Network::Tcp,
        doradus_core::DomainName::new("api.blocked.test").unwrap(),
        443,
    );
    assert_eq!(
...
fn not_domain_expression_compiles_to_an_exclusion_trie() {
    ...
    let blocked = Endpoint::domain(
        Network::Tcp,
        doradus_core::DomainName::new("www.blocked.example").unwrap(),
        443,
    );
    let allowed = Endpoint::domain(
        Network::Tcp,
        doradus_core::DomainName::new("other.example").unwrap(),
        443,
    );
    assert_eq!(
    ...
    assert_eq!(router.decide(&allowed).mode, doradus_core::RouteMode::Block);
}
...
fn not_any_uses_demorgan_and_preserves_network_and_port_constraints() {
    ...
    assert_eq!(router.decide(&tcp_80).mode, doradus_core::RouteMode::Block);
    assert_eq!(router.decide(&tcp_53).mode, doradus_core::RouteMode::Direct);
    assert_eq!(router.decide(&udp_80).mode, doradus_core::RouteMode::Direct);
}
...
fn hosts_as_host_and_global_network_rules_match_go_shapes() {
    ...
    assert_eq!(router.decide(&udp).mode, doradus_core::RouteMode::Block);
    let tcp = Endpoint::ip(Network::Tcp, "192.0.2.1:53".parse().unwrap());
    assert_eq!(router.decide(&tcp).mode, doradus_core::RouteMode::Direct);
}
