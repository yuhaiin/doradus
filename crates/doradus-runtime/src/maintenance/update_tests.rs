...
fn update_test_dir(name: &str) -> PathBuf {
    ...
    cache
        .join("doradus/update-tests")
        .join(format!("{name}-{}", std::process::id()))
}

#[tokio::test]
async fn production_update_service_reports_unsupported() {
    let service = UpdateService::new();
    assert_eq!(
        service.check("stable").await.unwrap_err(),
        "updates are currently unsupported"
    );
    assert_eq!(
        service.apply("stable", "v1.0.0").await.unwrap_err(),
        "updates are currently unsupported"
    );
}
...
fn release(tag: &str, prerelease: bool, published_at: &str) -> Release {
    Release {
        ...
        assets: vec![asset("doradus-linux-amd64"), asset("checksums.txt")],
    }
}
...
fn checksum_parser_accepts_gnu_and_bsd_style_filenames() {
    let body = "deadbeef\n";
    assert!(parse_checksum(body, "doradus-linux-amd64").is_err());
    let hash = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
    assert_eq!(
        parse_checksum(
            &format!("{hash}  doradus-linux-amd64\n"),
            "doradus-linux-amd64"
        )
        .unwrap(),
        hash
    ...
    assert!(
        parse_checksum(
            &format!("SHA256 (doradus-linux-amd64) = {hash}\n"),
            "doradus-linux-amd64"
        )
        .is_err()
    );
...
async fn check_fetches_release_metadata_and_checksum_from_http_fixture() {
    ...
    let hash = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
    let asset_name = format!("doradus-{}-{}", release_os(), release_arch());
