...
fn stop_platform_service() -> Result<(), String> {
    if let Ok(command) = env::var("DORADUS_UPDATE_STOP_COMMAND") {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        run_command(
            ...
            &[
                "bootout",
                "system",
                "/Library/LaunchDaemons/com.asutorufa.doradus.plist",
            ],
...
fn macos_launchd_pid() -> Result<Option<i32>, String> {
    let output = std::process::Command::new("launchctl")
        .args(["list", "com.asutorufa.doradus"])
        .output()
...
fn restart_platform_service() -> Result<(), String> {
    if let Ok(command) = env::var("DORADUS_UPDATE_RESTART_COMMAND") {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        return run_command(
            "launchctl",
            &["kickstart", "-kp", "system/com.asutorufa.doradus"],
...
fn windows_service() -> Result<windows_service::service::Service, String> {
    ...
    manager
        ...
        .map_err(|error| format!("open Windows service doradus: {error}"))
}
