...
const UPDATES_UNSUPPORTED: &str = "updates are currently unsupported";
...
impl UpdateService {
    pub fn new() -> Self {
        let mut service = Self::with_releases_url(String::new(), Duration::from_secs(30 * 60));
    ...
    fn request(
        ...
        let mut builder = Request::builder().method(method).uri(uri).header(
            header::USER_AGENT,
            format!("doradus/{}", env!("CARGO_PKG_VERSION")),
        );
    ...
    pub async fn check(&self, channel: &str) -> Result<CheckResult, String> {
        if !self.enabled {
            return Err(UPDATES_UNSUPPORTED.to_owned());
        }
