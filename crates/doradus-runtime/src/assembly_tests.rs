...
use doradus_store::{GoRouteListRecord, GoUdpProxyFqdnStrategy};
use doradus_trie::router::Router;
...
fn builder_publishes_route_list_contents_into_the_router_snapshot() {
    ...
    let endpoint = doradus_core::Endpoint::domain(
...
fn route_settings_repository_rows_are_loaded_by_runtime_reload() {
    ...
    block_on(
        store
            .repository()
            .put_go_route_settings(&doradus_store::GoRouteSettingsRecord {
                id: 1,
                direct_resolver: "direct".to_owned(),
                proxy_resolver: "proxy".to_owned(),
                resolve_locally: true,
