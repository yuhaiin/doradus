...
fn monitor_test_database_path() -> PathBuf {
    ...
    let directory = cache.join("doradus-monitor-tests");
...
fn monitor_reports_resolved_ip_separately_from_fakeip() {
    ...
    context.original_domain = Some(doradus_core::DomainName::new("www.google.com").unwrap());
...
fn monitor_preserves_route_explainability_metadata_in_connections() {
    ...
    context.match_history = vec![doradus_core::MatchHistoryEntry {
        rule_name: "media-rule".to_owned(),
        history: vec![doradus_core::MatchResult {
...
fn monitor_exposes_block_history_in_the_route_contract_shape() {
    ...
    context.original_domain = Some(doradus_core::DomainName::new("blocked.example").unwrap());
    ...
    second.original_domain = Some(doradus_core::DomainName::new("blocked.example").unwrap());
...
fn monitor_bounds_block_history_to_the_go_public_window() {
    ...
    for index in 0..=GO_HISTORY_SIZE {
        ...
        context.original_domain =
            Some(doradus_core::DomainName::new(&format!("blocked-{index}.example")).unwrap());
...
fn monitor_connection_uses_http_authority_for_placeholder_socket_tuple() {
    ...
    context.original_domain = Some(doradus_core::DomainName::new("example.test").unwrap());
    monitor.opened(flow, context);
...
fn monitor_connection_uses_socket_authority_for_resolved_udp_tuple() {
    ...
    let mut context = FlowContext::new(Endpoint::domain(
        Network::Udp,
        doradus_core::DomainName::new("example.test").unwrap(),
        443,
    ));
...
async fn failed_history_callback_does_not_wait_for_the_store_lock() {
    ...
    let lock_path = PathBuf::from(format!("{}-doradus-write-lock", path.display()));
...
fn monitor_force_abort_child() {
    let Some(path) = std::env::var_os("DORADUS_RUNTIME_MONITOR_CRASH_CHILD_PATH") else {
        return;
    };
