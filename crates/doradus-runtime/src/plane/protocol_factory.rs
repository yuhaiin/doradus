...
pub(super) async fn build_wireguard_proxy(
    ...
    let wireguard: doradus_wireguard::WireGuardConfig =
...
pub(super) async fn build_warp_masque_proxy(
    ...
    let warp: doradus_masque::WarpMasqueConfig = serde_json::from_value(layer.config.clone())
        .map_err(|error| {
            Error::new(
                ErrorKind::InvalidInput,
...
pub(super) async fn build_protocol_h2_proxy(
    ...
    let protocol = match config.transport {
        ...
        doradus_store::GoProxyTransport::Vmess => "vmess",
        doradus_store::GoProxyTransport::Trojan => "trojan",
        _ => {
...
pub(super) fn build_protocol_proxy(
    ...
    match config.transport {
        doradus_store::GoProxyTransport::Vless => {
            ...
            Ok(Arc::new(doradus_protocol::vless::VlessProxy::new(
                upstream, uuid,
            )?))
        }
        doradus_store::GoProxyTransport::Vmess => {
        ...
        doradus_store::GoProxyTransport::Trojan => {
            ...
            Ok(Arc::new(doradus_protocol::trojan::TrojanProxy::new(
                upstream, password,
            )))
        }
