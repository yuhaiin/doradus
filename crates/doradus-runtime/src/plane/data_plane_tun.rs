...
#[cfg(feature = "tun")]
pub async fn load_tun_config(store: &doradus_store::ConfigStore) -> Result<TunRuntimeConfig> {
...
fn parse_tun_routes(routes: &[String]) -> Result<Vec<doradus_tun::TunRoute>> {
    routes
        .iter()
        .map(|value| {
            ...
            doradus_tun::TunRoute::new(address, prefix)
...
#[cfg(feature = "tun")]
pub(crate) fn open_tun(config: &TunRuntimeConfig) -> Result<doradus_tun::TunRuntime> {
    let mut tun = doradus_tun::TunRuntime::open(config.tun.clone()).map_err(io_error)?;
    #[cfg(target_os = "macos")]
    {
    ...
    #[cfg(all(feature = "tun-routes", target_os = "macos"))]
    {
        ...
        {
            let portal_route = doradus_tun::TunRoute::new(IpAddr::V4(destination), prefix)?;
...
pub(super) fn tun_dns_servers(config: &doradus_tun::TunConfig) -> Vec<std::net::IpAddr> {
