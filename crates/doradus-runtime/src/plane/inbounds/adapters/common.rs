...
pub(crate) type UdpFlowId = doradus_types::InboundUdpFlowId;
...
const TEST_UDP_IDLE_TIMEOUT_ENV: &str = "DORADUS_TEST_UDP_IDLE_TIMEOUT_MS";
    ...
    B: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    let metadata = doradus_core::sniff::inspect(&sniffed);
...
fn dns_tcp_frame(bytes: &[u8]) -> Option<&[u8]> {
    ...
    doradus_core::dns::decode_query(packet).ok()?;
    Some(packet)
}
...
mod tests {
    ...
    async fn relay_hijacks_valid_dns_over_tcp_before_opening_outbound() {
        ...
        let query = doradus_core::dns::encode_query(
            17,
            &doradus_core::DomainName::new("example.com").unwrap(),
            doradus_core::dns::DnsRecordType::A,
        )
        .unwrap();
