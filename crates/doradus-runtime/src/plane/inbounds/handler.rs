...
pub(crate) use doradus_types::inbound::InboundDnsHandler;
...
impl InboundDnsPolicy {
    ...
    pub(crate) async fn answer_datagram(
        ...
        packet: &[u8],
    ) -> Option<doradus_core::Result<Vec<u8>>> {
    ...
    fn should_hijack(&self, destination_port: Option<u16>, packet: &[u8]) -> bool {
        self.monitor.dns_hijack_enabled()
            && (destination_port == Some(53)
                || doradus_core::dns::looks_like_supported_query(packet))
    }

    fn answer<'a>(
        &'a self,
        packet: &'a [u8],
    ) -> BoxFuture<'a, Option<doradus_core::Result<Vec<u8>>>> {
...
pub(crate) struct InboundInputInterceptor {
    ...
    dns_tasks: JoinSet<doradus_tun::ProxyInputAction>,
...
#[cfg(feature = "tun")]
impl doradus_tun::ProxyInputInterceptor for InboundInputInterceptor {
    fn intercept(
        &mut self,
        input: doradus_tun::ProxyInput,
    ) -> Result<doradus_tun::ProxyInputAction> {
        match input {
            doradus_tun::ProxyInput::UdpDatagram { flow, payload } => {
                let destination_port = Some(flow.key.destination.port());
                if !self.dns.should_hijack(destination_port, &payload) {
                    return Ok(doradus_tun::ProxyInputAction::Forward(
                        doradus_tun::ProxyInput::UdpDatagram { flow, payload },
                    ));
                }

                if self.dns_tasks.len() >= self.max_pending_dns {
                    return Ok(doradus_tun::ProxyInputAction::Drop);
                }
                ...
                self.dns_tasks.spawn(async move {
                    ...
                    match result {
                        Ok(Some(Ok(response))) => doradus_tun::ProxyInputAction::Reply {
                            flow: flow_key,
                            payload: response,
                        },
                        Ok(Some(Err(_))) | Ok(None) | Err(_) => doradus_tun::ProxyInputAction::Drop,
                    }
                });

                Ok(doradus_tun::ProxyInputAction::Deferred)
            }
            other => Ok(doradus_tun::ProxyInputAction::Forward(other)),
        }
    }

    fn wait_for_output<'a>(&'a mut self) -> BoxFuture<'a, doradus_tun::ProxyInputAction> {
        Box::pin(async move {
            loop {
...
impl InboundHandler {
    ...
    pub(crate) fn observe_datagram(&self, datagram: InboundDatagram) -> ObservedDatagram {
        let observation = doradus_core::flow::FlowObserverGuard::open(
            self.monitor.clone(),
...
impl doradus_types::InboundStreamHandler<BoxAsyncStream> for InboundHandler {
    fn handle_stream<'a>(
        &'a self,
...
impl<S> doradus_types::InboundStreamHandler<doradus_protocol::reverse_http::PrefixedIo<S>>
    for InboundHandler
where
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    fn handle_stream<'a>(
        &'a self,
        stream: doradus_protocol::reverse_http::PrefixedIo<S>,
        peer: SocketAddr,
        destination: Endpoint,
        protocol: &'static str,
    ) -> BoxFuture<'a, Result<()>> {
...
pub(crate) struct ObservedDatagram {
    pub(crate) datagram: Arc<dyn AsyncDatagram>,
    pub(crate) _observation: doradus_core::flow::FlowObserverGuard,
}
