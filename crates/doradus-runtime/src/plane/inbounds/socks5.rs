...
use doradus_core::{Network, Result};
...
pub(crate) async fn handle(
    ...
    let request = doradus_protocol::socks5_server::server_handshake(
        &mut stream,
        Network::Tcp,
        requires_auth,
        |username, password| {
    ...
    if matches!(
        request.command,
        doradus_protocol::socks5_server::Socks5Command::UdpAssociate
    ) {
        ...
        doradus_protocol::socks5_server::write_reply_endpoint(&mut stream, 0, advertised_address)
            .await?;
    ...
    let connection = match inbound.open_stream("socks5", peer, destination).await {
        ...
        Err(error) => {
            doradus_protocol::socks5_server::write_reply(&mut stream, 5).await?;
            return Err(error);
        }
    };
    doradus_protocol::socks5_server::write_reply(&mut stream, 0).await?;
    inbound
...
pub(crate) async fn serve_udp_socket(
    socket: Box<dyn doradus_protocol::socks5_server::UdpTransport>,
    inbound: Arc<InboundHandler>,
) -> Result<()> {
...
pub(crate) async fn serve_socks5_udp_loop(
    ...
    let codec = doradus_protocol::socks5_server::UdpServer::new(
        socket,
        allowed_peer,
...
impl doradus_protocol::socks5_server::UdpTransport for RuntimeUdpTransport {
    fn recv_from<'a>(
        &'a mut self,
        buffer: &'a mut [u8],
    ) -> Pin<Box<dyn Future<Output = io::Result<(usize, SocketAddr)>> + Send + 'a>> {
...
impl<S> InboundUdpFlowPolicy for doradus_protocol::socks5_server::UdpServer<S> where
    S: doradus_protocol::socks5_server::UdpTransport
{
}
