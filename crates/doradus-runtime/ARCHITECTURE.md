...
`doradus-runtime` is the data-plane runtime used by the management/API host.
...
chain/runtime policy. In Rust, `doradus-protocol` is the former boundary;
`doradus-runtime/src/plane/inbounds/adapters` is the latter. For example,
Trojan/VLESS/SOCKS5/Yuubinsya UDP servers and their wire messages live in
`doradus-protocol`, while runtime keeps authentication configuration, route
...
`doradus_protocol::http_server` owns the HTTP server loop and asks runtime's
...
`doradus_protocol::reverse_http` likewise owns sniffing, prefix restoration,
HTTP parsing and path/Host rewriting; runtime only routes, connects, wraps TLS,
relays and accounts bytes.
`doradus_protocol::transparent` owns Linux TPROXY/REDIRECT socket options,
...
`doradus-runtime::InboundUdpFlowPolicy` is intentionally separate because
...
in runtime even though protocol servers live in `doradus-protocol`.

The pure flow data contracts (`FlowContext`, `RouteMode`, `ResolverPolicy` and
route match records) also live in `doradus-types`; `doradus-core` re-exports
