...
`service_chain.rs` starts the real `doradus` executable built by Cargo, changes
...
- `api_reload_flow.rs` keeps one SQLite state and two real HTTP CONNECT
  ...
  `.cache/doradus/integration/api-reload-flow`.
...
- `make tun-reload-smoke` runs the same fixture with
  `DORADUS_TUN_RELOAD=1`: it changes the persisted Go inbound `enabled` field,
  ...
  `DORADUS_TUN_RELOAD_ONLY=1` (the Make target does this) to test lifecycle
...
- `scripts/integration/tun-chain-service.sh` runs the same real kernel TUN
  ...
  State and logs remain under `.cache/doradus/integration/tun-chain-service`;
...
- `scripts/integration/transparent-service.sh` runs an isolated privileged
  ...
  setting `DORADUS_TPROXY_ENABLED=1` now fails fast with a clear rootful/
...
- `scripts/integration/dns-source-bind.sh` runs the existing UDP/TCP resolver
  ...
  `.cache/doradus/integration/dns-source-bind`.
- `scripts/integration/doh-source-bind.sh` runs a real ring-backed DoH/HTTP2 and
  ...
  address; logs are kept under `.cache/doradus/integration/doh-source-bind`.
- `scripts/integration/socks5-udp-associate.sh` runs the real SOCKS5 control
  ...
  `.cache/doradus/integration/socks5-udp-associate`.
- `scripts/integration/node-latency-dns.sh` saves a direct node through the
  ...
  are kept under `.cache/doradus/integration/node-latency-dns`.
- `stats_concurrency.rs` starts the real runtime process, keeps an HTTP inbound
  ...
  under `.cache/doradus/integration/stats-concurrency`.
- `startup_logs.rs` starts the real runtime executable without
  `--quiet`, verifies that database/API/supervisor startup progress is
...
- `make go-protocol-interop-smoke` compiles the ignored cross-language
  ...
  mounted from `.cache/doradus`; the normal workspace test run does not
...
- `make go-termination-parity-smoke` starts the Go and Rust services with the
  ...
  remain under
  `.cache/doradus/integration/go-termination-parity`.
...
- `scripts/integration/api-contract.sh` runs the frontend management API
  ...
  logs are kept under `.cache/doradus/integration/api-contract`.
...
- standalone Go HTTP/2 transport wire compatibility is covered separately in
  `crates/doradus-chain/tests/standalone_http2.rs`: fixed endpoint resolution,
...
DORADUS_BENCH_BYTES=$((256 * 1024 * 1024)) make benchmark-throughput
```
...
`.cache/doradus/benchmarks/{http-throughput,tun-throughput,wireguard}/podman.log`.
...
```bash
cargo test -p doradus-runtime --all-features --offline --test api_contract -- --nocapture
cargo test -p doradus-runtime --all-features --offline --test service_chain -- --nocapture
scripts/integration/api-contract.sh
make service-chain-smoke
```

By default each test stores its SQLite state below
`.cache/doradus/integration/<scenario>/<pid>`. To retain a reusable
scenario directory for inspection or a Podman job, set an explicit cache path:

```bash
DORADUS_INTEGRATION_DIR=".cache/doradus/integration-reusable" \
  cargo test -p doradus-runtime --all-features --offline --test service_chain -- --nocapture
```

The fixed directory retains logs and fixture files for later inspection. To start
with a clean SQLite configuration, set `DORADUS_RESET_INTEGRATION_STATE=1`, or
...
DORADUS_TUN_BENCH_BYTES=$((16 * 1024 * 1024)) make benchmark-tun-throughput
```
