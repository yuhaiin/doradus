...
use doradus_store::{ConfigStore, install_go_snapshot};

fn cache_root() -> PathBuf {
    std::env::var_os("DORADUS_CACHE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(".cache/doradus"))
}
...
async fn real_go_v1_snapshot_builds_the_shared_runtime_snapshot() {
    ...
    install_go_snapshot(&path, &destination).await.unwrap();
    let store = ConfigStore::open(&destination).await.unwrap();
