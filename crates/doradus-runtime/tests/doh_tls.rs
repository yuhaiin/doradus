...
use doradus_core::proxy::{AsyncDatagram, AsyncProxy, AsyncProxySelector, BoxAsyncStream};
use doradus_core::{BoxFuture, DomainName, Error, ErrorKind, FlowContext, IpSet, ResolveStrategy};
use doradus_runtime::{
...
impl AsyncProxy for FixedTargetProxy {
    ...
    ) -> BoxFuture<'a, doradus_core::Result<BoxAsyncStream>> {
    ...
    fn open_datagram<'a>(
        &'a self,
        _context: &'a FlowContext,
    ) -> BoxFuture<'a, doradus_core::Result<Box<dyn AsyncDatagram>>> {
        Box::pin(async { Err(Error::new(ErrorKind::Unsupported, "test proxy has no UDP")) })
    }

    fn close(&self) -> BoxFuture<'_, doradus_core::Result<()>> {
        Box::pin(async { Ok(()) })
    }
}
...
async fn rustls_doh_rejects_bad_certificate_and_missing_h2() {
    ...
    let error = factory
        ...
        .await
        .unwrap_err();
    assert_eq!(error.kind, doradus_core::ErrorKind::Protocol);
...
async fn rustls_doh_cancels_a_slow_response_at_the_resolver_timeout() {
    ...
    let error = factory
        ...
        .await
        .unwrap_err();
    assert_eq!(error.kind, doradus_core::ErrorKind::Timeout);
}
