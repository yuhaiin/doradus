...
use yuhaiin_core::flow::{FlowDirection, FlowKey, FlowObserver, FlowObserverGuard};
use yuhaiin_core::{Endpoint, Error, ErrorKind, Result};
use yuhaiin_types::InboundDnsHandler;

pub(super) struct ObservedInbound {
    pub(super) source: SocketAddr,
    pub(super) observer: Arc<dyn FlowObserver>,
    pub(super) annotate: Arc<dyn Fn(&mut yuhaiin_core::FlowContext) + Send + Sync>,
}
