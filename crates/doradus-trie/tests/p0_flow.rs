...
use doradus_core::proxy::{AsyncDatagram, AsyncProxy, BoxAsyncStream, StaticProxySelector};
use doradus_core::{
    BoxFuture, DomainName, Endpoint, Error, ErrorKind, FlowContext, IpSet, Network, ResolverPolicy,
    Result, RouteMode,
};
use doradus_protocol::http::HttpProxy;
use doradus_protocol::proxy::{FixedAsyncProxy, Socks5AsyncProxy};
use doradus_store::ConfigStore;
use doradus_store::fakeip::{
    AsyncDomainResolver, FakeIpAnswerTransform, FakeIpAsyncDnsHandler, FakeIpConfig, FakeIpPool,
};
use doradus_tun::{ProxyInput, SmoltcpTunDevice, TunDispatcher, TunFlowKey, TunProxyRuntime};
...
impl AsyncDomainResolver for Resolver {
    ...
    ) -> doradus_core::BoxFuture<'a, Result<DnsResponse>> {
        Box::pin(async {
            Ok(DnsResponse {
                addresses: IpSet {
...
fn route_rule(pattern: &str) -> RouteRule {
    RouteRule {
        ...
        excluded_patterns: doradus_trie::HostTrie::new(),
...
async fn run_stream_proxy_tun(kind: StreamProxyKind) {
    ...
    let drop: Arc<dyn AsyncProxy> = Arc::new(doradus_protocol::proxy::DropAsyncProxy);
    let selector = Arc::new(doradus_core::proxy::StaticProxySelector {
...
async fn dns_fakeip_reverse_lookup_router_and_proxy_form_one_udp_flow() {
    ...
    runtime
        .handle_proxy_input(ProxyInput::UdpDatagram {
            flow: doradus_tun::TunFlow { key: flow.key },
