...
use doradus_protocol::proxy::{DirectAsyncProxy, DropAsyncProxy};
use doradus_protocol::websocket::WebSocketIo;
use doradus_protocol::yuubinsya::{
    YuubinsyaProtocol, decode_header, decode_uot_frame, derive_salt, encode_uot_frame,
};
use doradus_tun::{SmoltcpTunDevice, TunDispatcher, TunProxyRuntime};
use http::Response;
    ...
    ChainClient::new(ValidatedChain {
        ...
        nodes: vec![
            ValidatedNode::Fixed(ValidatedFixedConfig {
                addresses: vec![doradus_chain::ValidatedFixedAddress {
...
async fn chain_client_reuses_h2_pool_for_cached_ping() {
    ...
    let error = chain
        ...
        .await
        .unwrap_err();
    assert_eq!(error.kind, doradus_core::ErrorKind::Closed);
    chain.close().await;
    server.abort();
    let _ = server.await;
}
...
async fn chain_datagram_close_cancels_pending_recv() {
    ...
    let datagram: Arc<dyn doradus_core::proxy::AsyncDatagram> = proxy
    ...
    let result = tokio::time::timeout(Duration::from_secs(1), pending)
        ...
        .unwrap();
    assert_eq!(result.unwrap_err().kind, doradus_core::ErrorKind::Closed);
