...
impl RetryQueue {
    ...
    pub(super) fn acknowledge(&mut self, source: &Endpoint, payload: &[u8]) {
        ...
        let target = self.frames.iter().position(|frame| &frame.target == source);
...
impl ChainUotSession {
    ...
    pub(super) async fn send_to(&self, target: &Endpoint, payload: &[u8]) -> Result<()> {
        let frame = doradus_protocol::yuubinsya::encode_uot_frame(target, payload)?;
    ...
    async fn recv_from(&self) -> Result<(Endpoint, Vec<u8>)> {
        ...
        let (destination, payload, _) = doradus_protocol::yuubinsya::decode_uot_frame(&frame)?;
