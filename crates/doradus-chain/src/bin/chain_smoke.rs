...
use doradus_chain::{ChainClient, ValidatedNode, parse_config};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match mode.as_str() {
        "tcp" => {
            let target =
                env::var("DORADUS_CHAIN_TARGET").unwrap_or_else(|_| "example.com:443".to_owned());
            ...
            if env::var_os("DORADUS_CHAIN_PROBE").is_some() {
                session
        ...
        "uot" => {
            let target =
                env::var("DORADUS_CHAIN_TARGET").unwrap_or_else(|_| "example.com:53".to_owned());
            let destination = endpoint(&target, Network::Udp)?;
            let session = client.connect_uot(0).await?;
            let probe = env::var_os("DORADUS_CHAIN_PROBE").is_some();
