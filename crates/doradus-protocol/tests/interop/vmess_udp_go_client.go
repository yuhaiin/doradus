...
func main() {
	listen := os.Getenv("VMESS_UDP_LISTEN")
	...
	if err != nil {
		failf("VMess UDP packet conn: %v", err)
	}
	defer conn.Close()
	_ = conn.SetDeadline(time.Now().Add(3 * time.Second))
	if _, err := conn.WriteTo([]byte("go-vmess-udp"), target); err != nil {
		failf("VMess UDP write: %v", err)
	}
	response := make([]byte, 64)
	n, _, err := conn.ReadFrom(response)
	if err != nil {
		failf("VMess UDP read: %v", err)
	}
	if string(response[:n]) != "go-vmess-udp" {
		failf("VMess UDP response = %q", response[:n])
	}
}
