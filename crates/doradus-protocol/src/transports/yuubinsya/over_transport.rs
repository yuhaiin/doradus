...
use doradus_core::{BoxFuture, Endpoint, Error, FlowContext, Result};
...
impl YuubinsyaOverTransportProxy {
    pub fn new(
        ...
        if server.network() != doradus_core::Network::Udp || server.addr().is_none() {
...
impl AsyncProxy for YuubinsyaOverTransportProxy {
    fn connect<'a>(&'a self, context: &'a FlowContext) -> BoxFuture<'a, Result<BoxAsyncStream>> {
        Box::pin(async move {
            if context.network != doradus_core::Network::Tcp {
                return Err(Error::invalid(
    ...
    fn open_datagram<'a>(
        ...
        Box::pin(async move {
            if context.network != doradus_core::Network::Udp {
                return Err(Error::invalid(
