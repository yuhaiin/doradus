...
//!
//! The router only sees [`doradus_core::GeoLookup`].  This crate owns the
...
use doradus_core::{BoxFuture, Error, ErrorKind, GeoLookup, Result};
...
mod tests {
    ...
    fn cache_path(name: &str) -> PathBuf {
        ...
        root.join("doradus").join("geo-tests").join(name)
    }
    ...
    fn downloaded_country_without_asn_fixture_loads_and_queries() {
        let path = std::env::var_os("DORADUS_MAXMIND_FIXTURE")
            .map(PathBuf::from)
            .expect("DORADUS_MAXMIND_FIXTURE must point to a local .mmdb fixture");
        let db = GeoDb::open(path).unwrap();
        assert_eq!(
            db.country_code("2.125.160.217".parse().unwrap())
                .unwrap()
    ...
    fn temporary_files_are_created_next_to_target_and_tmp_is_rejected() {
        assert!(validate_path(Path::new("/tmp/doradus.mmdb")).is_err());
        assert!(validate_path(Path::new("/tmpfs/doradus.mmdb")).is_ok());
    }
}
