...
pub async fn tag_put_value(state: &ApiState, value: Value) -> ApiResult {
    ...
    let record = doradus_store::GoNodeTagRecord {
        id: name.clone(),
        name,
...
pub async fn node_latency_value(state: &ApiState, value: &Value) -> ApiResult {
    ...
    let resolver = snapshot
        .dns_resolver_for_route_mode(doradus_core::RouteMode::Direct)
    ...
    match tokio::time::timeout(
        outer_timeout,
        doradus_runtime::latency::probe_with_resolver(proxy, resolver, request, timeout),
    )
    .await
    {
