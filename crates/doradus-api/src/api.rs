//! Management HTTP API used by `doradus-react`.
//!
...
use doradus_backup::{S3Client, S3Config};
use doradus_core::proxy::AsyncProxy;
use doradus_core::{BoxFuture, DomainName, Endpoint, FlowContext, Network, ResolveStrategy};
use doradus_geo::{GeoDatabaseManager, GeoDownloadTransport, GeoRefreshRequest};
...
use doradus_runtime::update::UpdateService;
use doradus_runtime::{
...
impl From<doradus_core::Error> for ApiError {
    fn from(error: doradus_core::Error) -> Self {
        ...
        match error.kind {
            ...
            doradus_core::ErrorKind::InvalidInput | doradus_core::ErrorKind::Unsupported => {
                Self::bad(message)
            }
            doradus_core::ErrorKind::NotFound => Self::not_found(message),
            doradus_core::ErrorKind::Conflict => Self::conflict(message),
            // A closed/timeout runtime owner is equivalent to Go's
            // unavailable service dependency.
            doradus_core::ErrorKind::Closed | doradus_core::ErrorKind::Timeout => {
                Self::unavailable(message)
            }
            doradus_core::ErrorKind::Io
            | doradus_core::ErrorKind::Protocol
            | doradus_core::ErrorKind::Storage => Self::internal(message),
        }
    }
}
...
async fn pprof_heap(State(state): State<ApiState>) -> Response {
    ...
    Response::builder()
        ...
        .header(
            ...
            "attachment; filename=\"doradus-heap.pb.gz\"",
        )
        .body(Body::from(body))
...
async fn pprof_profile(State(state): State<ApiState>, Query(query): Query<PprofQuery>) -> Response {
    ...
    Response::builder()
        ...
        .header(
            header::CONTENT_DISPOSITION,
            "attachment; filename=\"doradus-profile.pb\"",
        )
        .body(Body::from(body))
        .unwrap_or_else(|error| {
            (
