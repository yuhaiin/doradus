...
impl GeoDownloadTransport for RouteGeoDownloadTransport {
    fn download<'a>(&'a self, url: &'a str) -> BoxFuture<'a, doradus_core::Result<Vec<u8>>> {
...
pub fn geo_cache_path() -> PathBuf {
    ...
    root.join("doradus").join("geo").join("Country.mmdb")
}
...
pub async fn refresh_geo_database(
    ...
    route: Arc<dyn RouteListTransport>,
    timeout: Duration,
) -> std::result::Result<Option<MaxMindMetadataRecord>, doradus_core::Error> {
    ...
    Ok(Some(MaxMindMetadataRecord {
        ...
        size: i64::try_from(metadata.size)
            .map_err(|_| doradus_core::Error::invalid("GeoIP file is too large to persist"))?,
...
pub async fn route_lists_refresh_value(state: &ApiState) -> ApiResult {
    ...
    let resolver = snapshot
        .dns_resolver_for_route_mode(doradus_core::RouteMode::Proxy)
