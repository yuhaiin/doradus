...
//!
//! `doradus-backup` owns SigV4 and deliberately knows nothing about the proxy
...
use doradus_backup::{BoxFuture, Error, S3Request, S3Response, S3Transport};
...
async fn tls_stream(host: &str, stream: BoxAsyncStream) -> Result<BoxAsyncStream, Error> {
    #[cfg(feature = "doh-tls")]
    {
        ...
        let dialer =
            doradus_runtime::RustlsTlsDialer::from_root_store(roots, Duration::from_secs(10))
...
mod tests {
    use super::*;
    use doradus_backup::S3Transport;
    use doradus_core::proxy::{AsyncDatagram, AsyncProxy};
    use doradus_core::{BoxFuture as CoreBoxFuture, ErrorKind as CoreErrorKind};
    use std::sync::Mutex;
    ...
    impl AsyncProxy for DuplexProxy {
        ...
        ) -> CoreBoxFuture<'a, doradus_core::Result<BoxAsyncStream>> {
            let stream = self.stream.lock().unwrap().take();
            Box::pin(async move {
                stream.ok_or_else(|| {
                    doradus_core::Error::new(CoreErrorKind::Closed, "duplex stream was reused")
                })
            })
        }
        ...
        fn open_datagram<'a>(
            &'a self,
            _context: &'a FlowContext,
        ) -> CoreBoxFuture<'a, doradus_core::Result<Box<dyn AsyncDatagram>>> {
            Box::pin(async {
        ...
        fn close(&self) -> CoreBoxFuture<'_, doradus_core::Result<()>> {
            Box::pin(async { Ok(()) })
        }
    }
