...
//!
//! The data plane remains in doradus-runtime; this module only owns the
//! executable's install/start/stop lifecycle without requiring a second
//! wrapper command. Linux, macOS and Windows use their
...
fn default_service_path() -> PathBuf {
    if cfg!(target_os = "windows") {
        PathBuf::from(r"C:\ProgramData\doradus")
    } else if cfg!(target_os = "macos") {
        PathBuf::from("/Library/Application Support/doradus")
    } else {
        PathBuf::from("/var/lib/doradus")
    }
}
...
mod tests {
    ...
    fn recognizes_launchctl_missing_service_errors() {
        assert!(is_missing_launchd_service(
            br#"Could not find service "com.asutorufa.doradus" in domain for system"#
        ));
