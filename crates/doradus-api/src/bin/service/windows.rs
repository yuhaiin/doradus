...
const SERVICE_NAME: &str = "doradus";
const TARGET_BIN: &str = r"C:\Program Files\doradus\doradus.exe";
...
fn install(args: &[OsString]) -> Result<()> {
    ...
    let result = (|| {
        service
            .set_description("doradus transparent proxy")
            .map_err(service_error)?;
        service
...
fn service_info(options: &ServiceOptions, target: &Path) -> ServiceInfo {
    ...
    ServiceInfo {
        ...
        display_name: OsString::from("doradus"),
...
fn windows_service_main(arguments: Vec<OsString>) {
    if let Err(error) = run_windows_service_inner(arguments) {
        eprintln!("doradus Windows service: {error}");
    }
}
