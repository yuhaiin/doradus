...
const TARGET_BIN: &str = "/usr/local/bin/doradus";
const SERVICE_PATH: &str = "/etc/systemd/system/doradus.service";
const SERVICE_NAME: &str = "doradus.service";
    ...
    if let Err(error) = result {
        ...
        return match rollback_error {
            ...
            Err(rollback_error) => Err(service_error(format!(
                "install failed: {error}; service recovery failed: {rollback_error}"
            ))),
        };
    }
    Ok(())
...
mod tests {
    ...
    fn service_unit_keeps_go_arguments_and_escapes_paths() {
        ...
        assert!(unit.contains(
            "ExecStart=/usr/local/bin/doradus -host 127.0.0.1:58080 -path /var/lib/doradus\\sdata -nfs-mode"
        ));
    ...
    fn service_backup_restores_previous_files_and_missing_entries() {
        let root = std::env::var_os("DORADUS_CACHE_DIR")
            ...
            .join("doradus/service-unit-tests")
            .join(format!(
                "{}-{}",
                std::process::id(),
