...
pub async fn route_rules_priority_value(state: &ApiState, value: &Value) -> ApiResult {
    ...
    result
        ...
        .map_err(|error| match error.kind {
            doradus_core::ErrorKind::NotFound => ApiError::not_found(error.to_string()),
            doradus_core::ErrorKind::InvalidInput => ApiError::bad(error.to_string()),
            _ => error.into(),
        })
}
...
pub async fn route_rules_test_value(state: &ApiState, value: &Value) -> ApiResult {
    ...
    let mode = match decision.mode {
        doradus_core::RouteMode::Direct => "direct",
        doradus_core::RouteMode::Proxy => "proxy",
        doradus_core::RouteMode::Bypass => "bypass",
        doradus_core::RouteMode::Block => "drop",
    };
