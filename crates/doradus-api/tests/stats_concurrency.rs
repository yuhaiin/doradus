...
async fn concurrent_stats_readers_survive_flow_updates_and_restart() {
    ...
    let reader_count = env_usize("DORADUS_STATS_READER_COUNT", 8);
    let reader_rounds = env_usize("DORADUS_STATS_READER_ROUNDS", 40);
    let write_rounds = env_usize("DORADUS_STATS_WRITE_ROUNDS", 64);
