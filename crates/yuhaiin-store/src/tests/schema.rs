...
fn schema_v1_migrates_to_typed_v3_without_losing_legacy_config() {
    let path = test_database_path();
    {
        ...
        connection
            .execute_batch(
                "CREATE TABLE yuhaiin_meta (
                    key TEXT PRIMARY KEY NOT NULL,
                    value INTEGER NOT NULL
                );
                CREATE TABLE yuhaiin_config (
                    key TEXT PRIMARY KEY NOT NULL,
                    value BLOB NOT NULL
                );
                INSERT INTO yuhaiin_meta (key, value)
                    VALUES ('schema_version', 1);
                INSERT INTO yuhaiin_config (key, value)
...
fn schema_v2_geo_migration_failure_rolls_back_and_retries_after_repair() {
    ...
    assert_eq!(error.kind, ErrorKind::Storage);
    {
        ...
        assert_eq!(
            connection
                .query("SELECT value FROM yuhaiin_config WHERE key = 'legacy.keep'")
                .unwrap()[0]
                .get(0),
...
fn legacy_table_rename_collision_rolls_back_and_retries_after_repair() {
    ...
    assert!(error.message.contains("both legacy and prepared"));
    {
        ...
        assert!(!table_exists(&connection, "yuhaiin_meta"));
...
fn base_schema_contract_mismatch_fails_closed_and_repairs_on_retry() {
    ...
    let error = match block_on(ConfigStore::open(&path)) {
        Ok(_) => panic!("base yuhaiin_config contract mismatch must fail closed"),
        Err(error) => error,
    };
    assert_eq!(error.kind, ErrorKind::Storage);
    assert!(error.message.contains("yuhaiin_config"));
...
fn future_schema_version_fails_closed_without_creating_typed_tables() {
    ...
    assert_eq!(
        connection
            .query("SELECT value FROM yuhaiin_config WHERE key = 'future.keep'")
            .unwrap()[0]
            .get(0),
