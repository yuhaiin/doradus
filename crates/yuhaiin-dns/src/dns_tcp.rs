...
mod async_tcp {
    ...
    use crate::dns_resolver::{AsyncDnsQuery, SendAsyncDnsQuery};
    ...
    impl AsyncTcpDnsClient {
        ...
        pub async fn query_packet(&self, packet: &[u8]) -> Result<Vec<u8>> {
            ...
            stream.set_nodelay(true).map_err(|error| {
                Error::new(ErrorKind::Io, format!("configure DNS TCP: {error}"))
            })?;
    ...
    impl<H: AsyncDnsHandler> AsyncTcpDnsServer<H> {
        pub async fn bind(
            ...
            let listener = TcpListener::bind(address).await.map_err(|error| {
                Error::new(ErrorKind::Io, format!("bind DNS TCP server: {error}"))
            })?;
            Ok(Self {
                listener,
                handler,
        ...
        async fn accept_from(&self) -> Result<(TcpStream, SocketAddr)> {
            let (stream, peer) =
                self.listener.accept().await.map_err(|error| {
                    Error::new(ErrorKind::Io, format!("accept DNS TCP: {error}"))
                })?;
            stream.set_nodelay(true).map_err(|error| {
                Error::new(ErrorKind::Io, format!("configure DNS TCP: {error}"))
            })?;
        ...
        fn async_tcp_client_forwards_unmodeled_qtypes_as_raw_packets() {
            ...
            runtime
                .block_on(async {
                    ...
                    let query =
                        encode_raw_query(0x6b6b, &DomainName::new("example.com").unwrap(), 16)?;
...
pub use async_tcp::{AsyncTcpDnsClient, AsyncTcpDnsHandler, AsyncTcpDnsServer};
