...
mod linux {
    ...
    fn invalid_interface_fails_without_installing_a_route() {
        ...
        let backend = LinuxTunRouteBackend::new("doradus-nonexistent-interface").unwrap();
    ...
    fn route_permission_error_fails_closed_without_a_lease() {
        ...
        assert!(matches!(
            ...
            doradus_tun::CapabilityState::Available
                | doradus_tun::CapabilityState::Unavailable
                | doradus_tun::CapabilityState::Unknown
        ));
        let route = TunRoute::new("198.18.0.0".parse::<IpAddr>().unwrap(), 15).unwrap();
    ...
    fn tun_open_with_routes_rolls_back_device_and_allows_recovery() {
        ...
        let route = TunRoute::new("198.18.0.0".parse().unwrap(), 15).unwrap();
        let backend = LinuxTunRouteBackend::new("doradus-route-does-not-exist").unwrap();
    ...
    fn tun_name_is_exclusive_and_reusable_after_process_stop() {
        ...
        let mut first = Command::new(&binary)
            .env("DORADUS_TUN_NAME", &name)
            .env("DORADUS_TUN_HOLD_MS", "10000")
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
            .unwrap();
