...
mod tests {
    ...
    impl AsyncProxy for BlackholeProxy {
        ...
        ) -> doradus_core::BoxFuture<'a, Result<BoxAsyncStream>> {
            Box::pin(async {
        ...
        fn open_datagram<'a>(
            &'a self,
            _context: &'a crate::FlowContext,
        ) -> doradus_core::BoxFuture<'a, Result<Box<dyn AsyncDatagram>>> {
    ...
    impl AsyncDatagram for BlackholeDatagram {
        ...
        fn recv_from<'a>(
            &'a self,
            _buffer: &'a mut [u8],
        ) -> doradus_core::BoxFuture<'a, Result<(usize, Endpoint)>> {
            Box::pin(pending())
        }
    ...
    async fn tcp_flow_stays_open_without_remote_progress_until_shutdown() {
        ...
        let worker = tokio::spawn(run_tcp_proxy(
            ...
            doradus_core::flow::Flow { key: flow }.context(),
            flow,
            commands_rx,
            output_tx,
            short_timeouts(),
