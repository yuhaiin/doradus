...
use doradus_core::RouteMode;
use doradus_core::nat::{NatKey, NatTable};
use doradus_core::process::{ProcessResolver, default_process_resolver};
#[cfg(test)]
pub use doradus_core::{BoxFuture, DomainName, IpSet};
use doradus_core::{Error, ErrorKind, Network, Result};
pub use doradus_core::{FlowContext, LocalBoxFuture};
#[cfg(test)]
pub use doradus_core::{dns, process};
#[cfg(test)]
pub mod proxy {
    pub use doradus_core::proxy::*;
    pub use doradus_protocol::proxy::{
        DelayedDropAsyncProxy, DirectAsyncProxy, DropAsyncProxy, FixedAsyncProxy,
    };
}

pub use doradus_core::flow::{Flow as TunFlow, FlowKey as TunFlowKey};
pub use doradus_core::flow::{FlowDirection as TunFlowDirection, FlowObserver as TunFlowObserver};

use doradus_core::proxy::{AsyncProxy, AsyncProxySelector, stream_local_addr, stream_remote_addr};
...
fn tun_debug(message: impl std::fmt::Display) {
    ...
    if *ENABLED.get_or_init(|| std::env::var_os("DORADUS_TUN_DEBUG").is_some()) {
        eprintln!("doradus: tun-debug: {message}");
    }
}
...
impl TunPcapCapture {
    fn from_env() -> io::Result<Option<Arc<Self>>> {
        let Some(path) = std::env::var_os("DORADUS_TUN_PCAP") else {
            return Ok(None);
        };
        if path.is_empty() {
            return Ok(None);
        ...
        eprintln!("doradus: TUN PCAP capture enabled: {}", path.display());
    ...
    fn record(&self, packet: &[u8]) {
        ...
        if let Err(error) = writer_ref.write_packet(packet) {
            *writer = None;
            eprintln!("doradus: TUN PCAP capture disabled: {error}");
        }
    }
}
