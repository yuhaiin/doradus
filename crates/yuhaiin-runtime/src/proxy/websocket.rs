//! WebSocket transport adapter used by inbound protocol listeners.
//!
//! Go places WebSocket below the actual inbound protocol: the HTTP upgrade is
//! consumed by the transport and the resulting binary message stream is then
//! handed to HTTP CONNECT, SOCKS5, Yuubinsya, or another protocol handler.
//! `WebSocketIo` exposes that message stream through Tokio's byte-oriented
//! `AsyncRead`/`AsyncWrite` boundary without buffering an unbounded amount of
//! data.
