...
- `cargo test -p yuhaiin-runtime --all-features --offline`：通过（82 个 runtime 单元测试 + 7 个 DoH/DoT 集成测试）；新增 `connections.close` 数字 ID fail-closed、monitor close event 唤醒、真实 SOCKS5 TCP relay 和 SOCKS5 UDP flow 关闭后 live connection 回收回归
