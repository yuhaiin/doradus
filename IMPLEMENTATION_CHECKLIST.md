...
| `[~]` | history/traffic/statistics | `monitor`, SQLite persistence | Rust checkpoint 负责频繁 crash recovery；无 checkpoint 时可接管 Go 的 `statistics_kv`、`traffic_hourly`、`connection_history`、`failed_connection_history` 和 telemetry 表；运行期间首次写入及每 30 秒低频写回 Go 兼容统计投影，正常 shutdown 再做最终原子投影；history 按 Go key 合并 | 生产库更多版本/异常中断 fixture；force-abort 后 Go 表可见性和跨进程读取仍需 Podman/进程级验收 |
