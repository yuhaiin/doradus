...
- `cargo test -p yuhaiin-runtime --all-features --offline data_plane -- --nocapture`：通过（3 tests）；外部 TUN host 可读取共享 `tun.runtime` 配置、IPv4/IPv6/MTU/队列和 selector ID，默认配置保持禁用且不会在加载阶段创建设备
