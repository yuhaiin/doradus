...
- [~] 连接观测扩展字段：共享 `FlowContext` 已保留 Go 兼容的 `hosts`、`tlsServerName`、`httpHost`、`interface`、`outboundGeo` 契约；core sniffer 已按 Go 规则解析 TLS ClientHello SNI 和 HTTP Host，共享 TCP relay 以 55ms 有界窥探并回放首包，覆盖 HTTP/SOCKS/Yuubinsya/Trojan/VLESS 等 raw relay，且在 `FlowObserverGuard` 建立前写入 monitor。runtime selector 现在会在同一份 route/proxy snapshot 中记录持久化 hosts 命中、解析 selected fixed/proxy/chain endpoint 的 outbound Geo，并通过本机地址 CIDR/Linux 最长前缀路由填充 interface；系统 `/etc/hosts`、协议内部只在握手后才可得的真实 socket metadata、Android/macOS interface backend 仍待补齐，不能用伪造值填充
...
- [~] 连接观测扩展字段：`FlowContext`/monitor 已提供 Go 字段契约；共享 TCP relay 已完成 Go 兼容的 TLS SNI/HTTP Host bounded sniff、55ms 超时和首包回放，并在 connection open 前记录 `tlsServerName/httpHost`；runtime selector 已接入持久化 hosts 命中、selected fixed/proxy/chain endpoint 的 Geo 和 Linux interface 路由解析。系统 hosts、握手后 socket 实际 interface 以及 Android/macOS 原生路径仍待补
