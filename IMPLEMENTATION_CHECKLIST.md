...
| `[~]` | Android | core 可注入 device，纯 Rust crate 设计不依赖 C TLS | target、VpnService fd、权限、电量/内存实测 |
| `[~]` | macOS | 部分 crate 可做 target compile boundary | utun、LaunchDaemon、权限、SIGTERM/route 实机验收 |
