...
- [~] 运行时 settings 应用：`settings` JSON 已解析为 immutable `RuntimeSettings` 并随 snapshot 原子 reload；`ipv6` 已统一作用于共享 resolver、按 ID resolver、DNS response 和 TUN portalV6，Go 的 pprof 默认值也已保留；`useDefaultInterface/netInterface` 的真实 socket bind、UDP/relay buffer、happy-eyeballs 并发和 pprof endpoint 仍待接入数据面
