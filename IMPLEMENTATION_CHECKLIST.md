...
8. 补管理 API 契约验收：逐个执行 generated client 的 list/detail/mutation 操作，记录 Rust 与 Go 的 response 字段、分页、query 过滤、错误状态和 reload side effect 差异；node/inbound/resolver 保存响应、route detail 默认值和 route activation 合并已有回归，继续覆盖剩余错误语义、长生命周期 apply 和真实 frontend snapshot。
