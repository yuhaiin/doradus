...
| `[~]` | history/traffic/statistics | `monitor`, SQLite persistence | Rust checkpoint 负责频繁 crash recovery；无 checkpoint 时可接管 Go 的 `statistics_kv`、`traffic_hourly`、`connection_history`、`failed_connection_history` 和 telemetry 表；运行期间首次写入及每 30 秒低频写回 Go 兼容统计投影，SQLite 锁导致的投影失败按 2 秒起、60 秒封顶指数退避，正常 shutdown 先收敛 persistence worker 再做最终原子投影；history 按 Go key 合并；独立文件库 reader 和真实子进程 `Child::kill` 已验证 checkpoint、Go 表及 WAL/sidecar 重开恢复 | 生产库更多版本/异常中断 fixture；还需在 Podman/真实部署中验证 Go 进程并发读写和升级期间统计表锁竞争 |
