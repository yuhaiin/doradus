...
按“功能能否工作”而不是“是否已经拿到所有真实设备/生产样本”统计，P1 业务闭环已经完成：FakeIP 双栈、DNS UDP/TCP/DoH/DoT、域名/CIDR Router、direct/fixed/drop/HTTP/SOCKS5/mixed/TLS/HTTP2/Yuubinsya TCP/UOT/UDP、单一路径 TUN、endpoint-independent Full Cone NAT、SQLite 配置/状态、Go v5/v6 兼容导入和 runtime reload 均已有实现与关联测试。入站认证语义已按 Go 对齐：SOCKS5/HTTP/Yuubinsya 使用 inbound protocol 自身凭据；mixed 入站会在同一 listener 上分派 SOCKS5 和 HTTP；HTTP 缺少凭据返回 407，凭据错误返回 403。`RuntimeSnapshot::new_full_cone_nat()` 还把持久化 Full Cone NAT timeout 接入了 TUN 组装，`RuntimeController::build_tun_proxy_runtime()` 进一步保证 selector、NAT 和 timeout 来自同一个 snapshot。
