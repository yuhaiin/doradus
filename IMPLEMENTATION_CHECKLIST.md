...
Fresh Rust 的 Go 默认投影已补齐：首次初始化同步 `settings_kv`、`route_extra`、`bootstrap.system=true`、LAN rule priority=1 和 Go 的 route preview，并移除不属于 Go `nodes_v2` 的 `rust-builtin/direct` 持久化节点；实际 Rust/Go fresh RPC 对照记录见 `MIGRATION.md` §53。宿主机已有 `127.0.0.1:1080` 时，API contract 的 active-node 进程 smoke 需放到 Podman 独立 network namespace，避免默认 mixed listener 端口冲突。
