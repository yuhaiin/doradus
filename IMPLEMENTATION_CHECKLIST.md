...
8. 补管理 API 契约验收：逐个执行 generated client 的 list/detail/mutation 操作，记录 Rust 与 Go 的 response 字段、分页、query 过滤、错误状态和 reload side effect 差异；node/inbound/resolver 的保存响应和 resolver system 规范化已有回归，继续优先覆盖 route、detail 和真实 frontend snapshot。
