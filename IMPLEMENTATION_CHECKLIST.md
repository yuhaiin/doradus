...
- `cargo test -p yuhaiin-protocol --test go_aead_interop --offline -- --ignored --nocapture`：通过（2 tests）；Go↔Rust AEAD TCP wire 互操作，Go 临时目录位于 `~/.cache/yuhaiin-rust/go-tmp`，不使用 `/tmp`
