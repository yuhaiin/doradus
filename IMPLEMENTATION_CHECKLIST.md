...
- [~] 连接观测扩展字段：共享 `FlowContext` 已保留 Go 兼容的 `hosts`、`tlsServerName`、`httpHost`、`interface`、`outboundGeo` 契约，monitor 不再硬编码为空，HTTP forward inbound 已记录真实 `Host`；TLS SNI/hosts resolver source/socket interface/selected outbound Geo 仍需由各数据面适配器提供真实来源，不能用伪造值填充
...
- [~] 连接观测扩展字段：`FlowContext`/monitor 已提供 Go 字段契约，HTTP forward 已写入 `httpHost`；TLS SNI、hosts 来源、实际 socket interface 和 outbound Geo 仍待接入对应 sniff/resolver/socket 生命周期
