...
| `make workspace-tests`（2026-08-12 当前轮） | Podman；宿主机只编译 harness | 48 个 harness；core 145、runtime 252、store 128、WireGuard 7、service-chain 15 全部通过；新增 TUN DNS hot-reload 和 WireGuard runtime UDP chain 回归通过；外部 WireGuard harness 的 2 个公网测试显式 ignored；另有 Debian VM rootful TUN/TPROXY 现场复核 |
