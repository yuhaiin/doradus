...
| `make workspace-tests`（2026-08-13 WireGuard resolver 修复） | 48 个 harness；编译和运行均在 Podman，日志位于 `~/.cache/yuhaiin-rust/integration/workspace-tests` | chain 55、core 148、runtime 281、store 131（5 ignored）、trie 27、service-chain 20、WireGuard 8（1 benchmark ignored）、WireGuard runtime chain 2、stats concurrency 2；0 失败；外部 WARP 2 项按需配置显式 ignored |
...
| `make wireguard-smoke && make wireguard-chain-smoke`（2026-08-13 WireGuard resolver 修复） | BoringTun 单测和 runtime chain 均在 Podman；缓存/日志位于 `~/.cache/yuhaiin-rust` | WireGuard protocol `8 passed, 0 failed, 1 ignored`；runtime HTTP/TCP 与 SOCKS5/UDP chain `2/2` 通过 |
