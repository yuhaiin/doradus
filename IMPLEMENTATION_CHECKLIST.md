...
- [x] **P2-BINARY-01：** `yuhaiin` binary 直接启动 HTTP listener；默认创建内置 direct node，支持 `YUHAIIN_DB`、`YUHAIIN_HTTP`、`YUHAIIN_TUN`，生命周期由 Ctrl-C/watch shutdown 收敛；无默认 feature 的 `http-api` 最小构建也不会错误导入 TUN API。
