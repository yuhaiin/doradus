...
| 状态 | 功能 | 位置 | 当前结果 | 剩余工作 |
| --- | --- | --- | --- | --- |
| `[~]` | 前端 API/RPC | `runtime/src/api.rs` | 对齐现有 generated client；settings、nodes、inbounds、DNS、hosts/FakeDNS、route、TUN、connections 等共用 store/runtime struct；列表 API 已补 Go 的 query 字段边界并在过滤后计算 total；核心错误已按 Go RPC 分类为 400/404/503/500 | 继续用前端 generated-contracts 和真实 Go handler 做 response 字段/剩余错误语义逐项验收；补 production snapshot 的管理面回归 |
