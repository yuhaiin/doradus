...
- `[x]` 2026-08-13 在 Podman 重跑 `make go-live-flow-parity-smoke` 和 `make workspace-tests`：Go/Rust 真实 live flow 的 connections、total、traffic、telemetry、history 对照通过；workspace 49 个 harness 全部通过（289 个 runtime tests、132 个 store tests、24 个 service-chain tests、2 个 WireGuard chain tests）。结果保存在 `~/.cache/yuhaiin-rust/integration/go-live-flow-parity/20260813034332-2381346` 与 `~/.cache/yuhaiin-rust/integration/workspace-tests`。
...
- `[x]` Go `network_split` 已恢复 TCP/UDP 分支和父链前缀语义：分支支持 direct/fixed/drop/reject/HTTP/HTTP proxy/SOCKS5/TLS/WebSocket/协议层/Yuubinsya，HTTP/2 分支会像 Go `ContractWrap` 一样把父代理作为 plaintext prior-knowledge H2 dialer，UDP 仍委托父代理；父链 fixed endpoint 会继续填充前端 `connections.outbound`。nested split 和 WireGuard branch 仍在无法安全组合时显式拒绝。Podman focused tests 和真实 HTTP inbound→router→`fixed→network_split`→HTTP proxy→echo service-chain 均通过。
...
| 类别 | 命令 | Podman 验证结果 |
| --- | --- | --- |
| 全 workspace | `make workspace-tests` | 49 个 harness，0 失败；chain 55、core 154、runtime 289、store 132（5 ignored）、trie 27、service-chain 24、WireGuard 11（1 benchmark ignored）、WireGuard runtime chain 2、stats concurrency 2；外部 WARP 测试显式 ignored；新增 RuntimeDnsHandler→UDP/TCP server/client 原始 QTYPE 回归 |
