...
| live inbound/reload | `make api-reload-flow-smoke tun-api-process-smoke` | Podman `network=none` API mutation/reload/restart flow 2/2；真实前台二进制通过 API 开启、关闭并持久化 TUN 设备 1/1，确认 TUN 与普通 inbound 共用 reload supervisor |
