...
| `pkg/net/proxy/network_split/*` | `crates/yuhaiin-store/src/compat_proxy.rs`、`crates/yuhaiin-store/src/compat_proxy_async.rs`、`crates/yuhaiin-runtime/src/proxy.rs` | `[x]` | 按 Go 语义分别构造 TCP/UDP branch，保留父链前缀；direct/fixed/drop/reject/HTTP/HTTP proxy/SOCKS5/TLS/WebSocket/协议层/Yuubinsya branch 已接入；父链 fixed endpoint 会用于 connections/tag metadata；Podman runtime/store focused tests 和真实 HTTP inbound→router→`fixed→network_split(tcp=http,udp=drop)`→HTTP proxy→echo service-chain 通过；nested `network_split`、不完整 HTTP/2 branch 和 WireGuard branch 显式报错 | — |
...
- `[x]` TUN 集成脚本的主服务和 MTU 矩阵现在支持 `YUHAIIN_TEST_IMAGE` 覆盖容器发行版；使用 Podman 编译的 `x86_64-unknown-linux-musl` 静态 PIE 在 Alpine 中完成 reload、无路由窗口、真实 packet traffic 和 close 回归，并以 576/1280/1500/9000/9216 五档 MTU 回环 65507 字节 UDP，证明 harness 不依赖 Debian 用户态。更广泛宿主 firewall/发行版 kernel 组合仍保留为 `[~]`。
...
- `[x]` 2026-08-13 在 Podman 重跑 `make go-live-flow-parity-smoke` 和 `make workspace-tests`：Go/Rust 真实 live flow 的 connections、total、traffic、telemetry、history 对照通过；workspace 49 个 harness 全部通过（288 个 runtime tests、132 个 store tests、23 个 service-chain tests、2 个 WireGuard chain tests）。结果保存在 `~/.cache/yuhaiin-rust/integration/go-live-flow-parity/20260813034332-2381346` 与 `~/.cache/yuhaiin-rust/integration/workspace-tests`。
...
- `[x]` Go inbound `proxy` 与 `http_mock` transport 已按其真实 `NewServer` 语义复用普通 listener；allow-list 单测覆盖大小写和 deferred transport，Podman `service-chain-smoke` 的 18/18 进程链包含两种透明 wrapper 的 HTTP echo。Go outbound `none` no-op point 也已映射为仅含 `none` 节点的 direct/zero proxy；混入未知未来协议时仍保留 unknown，不静默丢层。
- `[x]` Go `network_split` 已恢复 TCP/UDP 分支和父链前缀语义：分支支持 direct/fixed/drop/reject/HTTP/HTTP proxy/SOCKS5/TLS/WebSocket/协议层/Yuubinsya，父链 fixed endpoint 会继续填充前端 `connections.outbound`；nested split、独立 HTTP/2 branch、WireGuard branch 等无法安全组合的情况显式拒绝。Podman focused tests 和真实 HTTP inbound→router→`fixed→network_split`→HTTP proxy→echo service-chain 均通过。
...
| 类别 | 命令 | Podman 验证结果 |
| --- | --- | --- |
| 全 workspace | `make workspace-tests` | 49 个 harness，0 失败；chain 55、core 154、runtime 288、store 132（5 ignored）、trie 27、service-chain 23、WireGuard 11（1 benchmark ignored）、WireGuard runtime chain 2、stats concurrency 2；外部 WARP 测试显式 ignored；新增 RuntimeDnsHandler→UDP/TCP server/client 原始 QTYPE 回归 |
| NetworkSplit 兼容 | `podman-cargo.sh -- cargo test -p yuhaiin-runtime --all-features network_split`；`podman-cargo.sh -- cargo test -p yuhaiin-store --all-features go_network_split_runtime_keeps_branch_layers_and_prefix_transport` | runtime TCP/UDP branch dispatch 与 Go-shaped config/prefix transport 两项均通过；测试状态和构建缓存位于 `~/.cache/yuhaiin-rust/integration/network-split/`、`~/.cache/yuhaiin-rust/cargo-target/` |
...
| statistics soak | `YUHAIIN_STATS_READER_COUNT=32 YUHAIIN_STATS_READER_ROUNDS=1200 YUHAIIN_STATS_WRITE_ROUNDS=3000 make stats-soak-smoke` | Podman 真实前台进程 32 readers×1200 rounds、3000 writes，含强停恢复：2 passed，132.56s；此前 24×1000、2000 writes 也通过 |
