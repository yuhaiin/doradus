...
- [~] 连接观测扩展字段：共享 `FlowContext` 已保留 Go 兼容的 `hosts`、`tlsServerName`、`httpHost`、`interface`、`outboundGeo` 契约；core sniffer 已按 Go 规则解析 TLS ClientHello SNI 和 HTTP Host，共享 TCP relay 以 55ms 有界窥探并回放首包，覆盖 HTTP/SOCKS/Yuubinsya/Trojan/VLESS 等 raw relay，且在 `FlowObserverGuard` 建立前写入 monitor。runtime selector 现在会在同一份 route/proxy snapshot 中合并系统 hosts 与持久化 hosts（持久化配置优先）、解析 selected fixed/proxy/chain endpoint 的 outbound Geo，并通过本机地址 CIDR/Linux 最长前缀路由填充 interface；协议内部只在握手后才可得的真实 socket metadata、Android/macOS interface backend 仍待补齐，不能用伪造值填充
- [~] DNS resolver/server、UDP、TCP fallback、DoH/DoT（wire、Go resolver runtime transport 解析、统一同步/异步 `DnsResolver` facade、同步/异步 UDP client/server、纯 Tokio 异步 RFC 1035 TCP client/handler/server、持久 TCP connection 多 frame、并发 connection accept、runtime DNS supervisor 同时监听配置地址的 UDP/TCP、异步 H2 DoH query adapter、RFC 1035 TCP/DoT length-prefix client/server、`AsyncUdpDnsHandler` packet adapter、`AsyncUdpDnsServer`、A/AAAA/PTR/HTTPS/SVCB query decode/response encode、未知 SvcParam 保留、同步/异步 handler boundary、policy、owner-future cancellation、异步 DNS upstream timeout、HTTP/2 DoH framing、`H2DohDnsHandler` packet adapter、TTL/容量 DNS cache、可复用 `HostsTable` A/AAAA override、同步/异步 hosts handler、TUN DNS hijack 和 FakeIP 组合闭环已完成；runtime TCP resolver 已切换到 `AsyncTcpDnsClient`，不再为每次 TCP fallback 占用 `spawn_blocking` 线程；`yuhaiin-runtime` 新增可注入 TLS/proxy connector 的 `H2DohResolverFactory`，以及复用 RustCrypto TLS dialer 的 `RustCryptoH2Connector`/`RustCryptoDohResolverFactory`/`RustCryptoDotResolverFactory`/`RustCryptoResolverFactory`，后者可在同一 registry 中混用 DoH 与 DoT；`run_dns_supervisor` 现在优先使用 Rust overlay、否则读取 Go `dns_settings.server`，resolver server PUT 也回写旧表；真实 TLS/H2 DoH、TLS/DoT framing、证书/ALPN/HTTP 状态/content-type/响应超时、混合 registry 和 runtime UDP/TCP listener 回归已通过；DoQ/DoH3 和 P2 平台路径仍待补
- [~] 运行时 settings 应用：Rust `settings` JSON overlay 与 Go `settings_kv(section,key,value_json)` 兼容读取/已知 scalar 回写均已接入 immutable `RuntimeSettings` 和 snapshot 原子 reload；`ipv6` 已统一作用于共享 resolver、按 ID resolver、DNS response 和 TUN portalV6，Go 的 pprof 默认值也已保留；`relayBufferSize` 已作用于 inbound TCP relay，`udpBufferSize/udpRingbufferSize` 已作用于 SOCKS5/Trojan/VLESS/Yuubinsya UDP loop 与 runtime DNS UDP listener，`happyEyeballsSemaphore` 已作为 runtime TCP connection-establishment budget 并随 live selector reload；`useDefaultInterface/netInterface` 的真实 socket bind 和 pprof endpoint 仍待接入数据面，当前 budget 不是完整 IPv4/IPv6 racing 实现
...
- [~] 连接观测扩展字段：`FlowContext`/monitor 已提供 Go 字段契约；共享 TCP relay 已完成 Go 兼容的 TLS SNI/HTTP Host bounded sniff、55ms 超时和首包回放，并在 connection open 前记录 `tlsServerName/httpHost`；runtime selector 已接入系统 hosts + 持久化 hosts 优先级、selected fixed/proxy/chain endpoint 的 Geo 和 Linux interface 路由解析。仍待握手后 socket 实际 interface 以及 Android/macOS 原生路径
